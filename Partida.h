...
public:
	...
	Partida(ModeJoc mode) : m_mode(mode), m_temps(0.0), m_punts(0), m_nivell(1), m_velocitat(VELOCITAT_INICIAL), m_finalJoc(false), m_finalJocY(0)
	{
		srand(time(0));
	}
...
private:
	...
	bool m_finalJoc;
	float m_finalJocY;
	const float FINAL_JOC_TARGET_Y = 300;
	const float FINAL_JOC_SPEED = 100;
