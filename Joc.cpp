...
bool Joc::novaFigura()
{   
	m_figuraActual = m_figuraSeguent;
	ProximaFigura();
	bool colisiona = m_tauler.colisionaFigura(m_figuraActual);
	m_figuraSeguent.dibuixaNext();
	return colisiona;
}
void Joc::ProximaFigura() 
{
	...
	int nGir = (rand() % 4);
	m_figuraSeguent.inicialitza(tipus, 1, columna);
	for (int i = 0; i < nGir; i++)
		m_figuraSeguent.gira(GIR_HORARI);


}
void Joc::generaSeguent() 
{
	m_figuraSeguent.dibuixaNext();
