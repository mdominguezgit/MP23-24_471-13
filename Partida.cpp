...
void Partida::actualitza(float deltaTime)
{
    ...
    string Seguent = "Seguent Figura ";
    string msgPunts = "Puntuacio: " + to_string(m_punts);
    string msgNivell = "Nivell: " + to_string(m_nivell);
    GraphicManager::getInstance()->drawFont(FONT_WHITE_30, POS_X_NEXTFIG, 20, 0.5, Seguent);
    ...
    if (m_finalJoc)
    {
        if (m_finalJocY < FINAL_JOC_TARGET_Y) {
            m_finalJocY += FINAL_JOC_SPEED * deltaTime;
            if (m_finalJocY > FINAL_JOC_TARGET_Y) {
                m_finalJocY = FINAL_JOC_TARGET_Y;
            }
        }
        string msgFinal = "GAME OVER";
        GraphicManager::getInstance()->drawFont(FONT_RED_30, POS_X_FINAL_JOC, m_finalJocY, 1.8, msgFinal);
